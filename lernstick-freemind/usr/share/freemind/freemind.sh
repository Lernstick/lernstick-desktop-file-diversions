...
then
	...
	"${JAVA_CMD}" -Xmx256M -Dawt.useSystemAAFontSettings=on -Dfreemind.base.dir="${freedir}" -cp "${CLASSPATH}" freemind.main.FreeMindStarter "$@"
	...
	( echo "${DEBUG}" | grep -qe "exit" ) && exit 0 # do not start FreeMind
	"${JAVA_CMD}" -Xmx256M -Dawt.useSystemAAFontSettings=on -Dgnu.java.awt.peer.gtk.Graphics=Graphics2D -Dfreemind.base.dir="${freedir}" -cp "${CLASSPATH}" freemind.main.FreeMindStarter "$@"
fi
